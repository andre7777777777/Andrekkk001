...
def bloomberg():
    base_url = 'https://www.bloomberg.com/politics'
    ...
    if r.status == 200:
        ...
        soup = s.find_all('article', class_='story-package-module__story mod-story')
        soup += s.find_all('article', class_='story-list-story mod-story')
        ...
        for i in soup:
            if 'ukraine' in i.get_text().lower() or 'ukrainian' in i.get_text().lower():
                if i not in soups:
                    img = i.find('img')
                    i = i.find('a', class_='story-package-module__story__headline-link')

                    soups.append([i['href'], img])

        for i in soups:
            r = requests.get('https://www.bloomberg.com' + i[0], headers=headers)
            ...
            if r.status == 200:
                ...
                s = s.find('div', class_='body-content')

                post = s.find_all('h1', class_='headline__699ae8fb') + s.find_all('p')
                post = [i.get_text() for i in post]
